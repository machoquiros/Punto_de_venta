...
if ($_POST['insertar'])
{
$conec = pg_connect("host=localhost  dbname=Punto_Venta user=postgres password=ECCIpsql2014 ") or die("problemas de coneccion " . pg_last_error());
...
header('Location: http://localhost/Punto_Venta/Producto.html');
}
if ($_POST['modificar'])
{
...
$result = pg_query($conec,$query) or die('La consulta fallo: ' . pg_last_error());
	...
	while ($row = pg_fetch_row($result))
	{
		...
		echo '</body>';
		echo '</html>';
		//header('Location: http://localhost/Punto_Venta/Productob.html?nombre='.$row[0]);
	}
}
